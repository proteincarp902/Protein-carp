...
/* ===== Mobile Premium V1.3 ===== */
function pcMobileDateKeyFromMs(ms){if(!ms)return"";const d=new Date(ms);return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;}
function pcMobileTodayKey(){const d=new Date();return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;}
function pcMobileDateKey(x){return pcMobileDateKeyFromMs(getTimeValue(x));}
function pcTodayProductionLogsForChef(){if(!currentChef)return[];const key=pcMobileTodayKey();return(productionLogs||[]).filter(log=>{const same=String(log.chefCode||"")===String(currentChef.code||"")||String(log.chefName||"")===String(currentChef.name||"");return same&&pcMobileDateKey(log)===key;});}
function pcTodayWasteLogsForChef(){if(!currentChef)return[];const key=pcMobileTodayKey();return(wasteLogs||[]).filter(log=>String(log.chefName||"")===String(currentChef.name||"")&&pcMobileDateKey(log)===key);}
function pcChefTodayStatus(){const logs=pcTodayProductionLogsForChef();if(!logs.length)return{text:"معلق",cls:"pending",qty:0,logs};if(logs.some(x=>x.noProduction))return{text:"لا يوجد إنتاج",cls:"none",qty:0,logs};let qty=0;logs.forEach(log=>(log.items||[]).forEach(i=>qty+=Number(i.qty||0)));return{text:"تم رفع الإنتاج",cls:"done",qty,logs};}
function pcMenuItemsForChef(){const section=currentChef?.section||"";return(menuItems||[]).filter(i=>!i.section||i.section===section);}
function pcWarehouseItemsForChef(){const section=currentChef?.section||"";return(warehouseItems||[]).filter(i=>!i.section||i.section===section);}
function pcChefHeader(title=""){const cartCount=(currentCart||[]).reduce((s,i)=>s+Number(i.qty||0),0);const st=pcChefTodayStatus();return `<section class="chef-app-header"><button class="chef-icon-btn" onclick="smartBack()"><i class="fa-solid fa-arrow-right"></i></button><div class="chef-header-text"><span>${escapeHtml(currentChef?.section||title||"")}</span><b>${escapeHtml(currentChef?.name||"")}</b><small class="${st.cls}">${st.text}</small></div><button class="chef-cart-icon" onclick="renderWarehouseRequest()"><i class="fa-solid fa-cart-shopping"></i>${cartCount?`<em>${cartCount}</em>`:""}</button></section>`;}
function renderChefDashboard(chef){currentChef=chef;productionDraft=productionDraft||[];currentCart=currentCart||[];const st=pcChefTodayStatus();const wasteCount=pcTodayWasteLogsForChef().length;pageLayout(chef.name,`${pcChefHeader()}<section class="chef-app-hero"><div><span>واجهة الشيف</span><h2>${escapeHtml(chef.section||"")}</h2><p>سجل إنتاج اليوم والهدر، واطلب احتياجك من السلة بالأعلى.</p></div></section><section class="chef-app-actions"><button class="chef-app-card production" onclick="renderProduction()"><span>إنتاج اليوم</span><b>${st.qty||0}</b><small>${st.text}</small></button><button class="chef-app-card waste" onclick="renderWaste()"><span>الهدر والتالف</span><b>${wasteCount}</b><small>تسجيل هدر جديد</small></button></section><section class="chef-app-panel"><div class="chef-panel-head"><h3>آخر إنتاج مرفوع اليوم</h3>${st.cls==="pending"?`<button class="chef-mini-btn" onclick="submitNoProductionToday()">لا يوجد إنتاج</button>`:""}</div>${st.logs.length?`<div class="chef-last-list">${st.logs.slice(0,4).map(log=>`<div class="chef-last-row"><b>${escapeHtml(log.createdAtText||"اليوم")}</b><small>${log.noProduction?"لا يوجد إنتاج اليوم":((log.items||[]).map(i=>`${escapeHtml(i.name||i.productName||"")} : ${escapeHtml(i.qty||"")}`).join(" • ")||"إنتاج مرفوع")}</small></div>`).join("")}</div>`:`<div class="chef-empty">لم يتم رفع إنتاج اليوم بعد</div>`}</section><section class="chef-bottom-actions"><button onclick="renderMyOrders()"><i class="fa-solid fa-clipboard-list"></i><span>طلباتي</span></button><button onclick="renderWarehouseRequest()"><i class="fa-solid fa-cart-shopping"></i><span>طلب مستودع</span></button></section>`,"renderChefs()");}
function renderProduction(){if(!currentChef)return renderChefs();const sectionItems=pcMenuItemsForChef();const options=sectionItems.map(i=>`<option value="${escapeHtml(i.nameAr||i.name||"")}">${escapeHtml((i.code?i.code+" - ":"")+(i.nameEn||""))}</option>`).join("");const st=pcChefTodayStatus();pageLayout("الإنتاج",`${pcChefHeader("الإنتاج")}<section class="chef-app-panel"><div class="chef-panel-head"><h3>إنتاج اليوم</h3><span class="chef-status-chip ${st.cls}">${st.text}</span></div><label>الصنف</label><input id="productionNameInput" list="productionMenuList" placeholder="${sectionItems.length?"ابحث في أصناف "+escapeHtml(currentChef.section):getProductionPlaceholder()}" onkeydown="focusProductionQty(event)"><datalist id="productionMenuList">${options}</datalist><label>الكمية</label><input id="productionQtyInput" type="number" min="1" placeholder="مثال: 20" onkeydown="handleProductionQtyInput(event)"><button class="btn btn-main chef-wide-btn" onclick="addProductionFromInputs()">إضافة للإنتاج</button><textarea id="productionNote" placeholder="ملاحظة للإدارة"></textarea></section><section class="chef-app-panel"><h3>الإنتاج الحالي</h3><div id="productionDraftBox"></div><div class="chef-action-row"><button class="btn btn-main" onclick="submitProduction()">رفع الإنتاج</button><button class="btn btn-light" onclick="submitNoProductionToday()">لا يوجد إنتاج</button></div></section><div id="lastProductionBox" style="margin-top:16px"></div>`,"renderChefDashboard(currentChef)");drawProductionDraft();drawLastProductionForChef();setTimeout(()=>document.getElementById("productionNameInput")?.focus(),100);}
async function submitNoProductionToday(){if(!currentChef)return;if(!confirm("تأكيد تسجيل: لا يوجد إنتاج لهذا اليوم؟"))return;const{db,addDoc,collection,serverTimestamp}=window.firebaseDB;await addDoc(collection(db,"production_logs"),{chefName:currentChef.name,chefCode:currentChef.code||"",section:currentChef.section,items:[],note:"لا يوجد إنتاج اليوم",noProduction:true,createdAt:serverTimestamp(),createdAtMs:Date.now(),createdAtText:nowText()});productionDraft=[];showToast("تم تسجيل لا يوجد إنتاج اليوم");renderChefDashboard(currentChef);}
function renderWarehouseRequest(){if(!currentChef)return renderChefs();currentCart=currentCart||[];pageLayout("طلب مستودع",`${pcChefHeader("طلب مستودع")}<section class="warehouse-mobile-layout"><div class="chef-app-panel"><div class="chef-panel-head"><h3>طلب من المستودع</h3><span class="chef-status-chip">${escapeHtml(currentChef.section||"")}</span></div><input id="warehouseRequestSearch" placeholder="ابحث عن صنف المستودع" oninput="drawWarehouseRequestItemsMobile()"><div id="warehouseRequestItemsBox"></div></div><div class="chef-app-panel warehouse-cart-panel"><div class="chef-panel-head"><h3>السلة</h3><span class="chef-status-chip">${currentCart.length}</span></div><div id="warehouseCartBox"></div><textarea id="warehouseOrderNote" placeholder="ملاحظة للمستودع"></textarea><button class="btn btn-main chef-wide-btn" onclick="submitWarehouseOrder()">إرسال الطلب</button></div></section>`,"renderChefDashboard(currentChef)");drawWarehouseRequestItemsMobile();drawWarehouseCart();}
function drawWarehouseRequestItemsMobile(){const box=document.getElementById("warehouseRequestItemsBox");if(!box)return;const q=(document.getElementById("warehouseRequestSearch")?.value||"").trim().toLowerCase();const list=pcWarehouseItemsForChef().filter(i=>String(i.name||i.nameAr||"").toLowerCase().includes(q)||String(i.nameEn||"").toLowerCase().includes(q)||String(i.code||"").toLowerCase().includes(q)).slice(0,80);if(!list.length){box.innerHTML=`<div class="chef-empty">لا توجد أصناف مستودع لهذا القسم</div>`;return;}box.innerHTML=`<div class="warehouse-mobile-items">${list.map(i=>`<div class="warehouse-mobile-item"><div><b>${escapeHtml(i.nameAr||i.name||"")}</b><small>${escapeHtml(i.nameEn||i.code||"")}${i.unit?" • "+escapeHtml(i.unit):""}</small></div><button onclick="addWarehouseItemToCart('${i.id}')"><i class="fa-solid fa-plus"></i></button></div>`).join("")}</div>`;}
function addWarehouseItemToCart(id){const item=(warehouseItems||[]).find(i=>i.id===id);if(!item)return;currentCart=currentCart||[];const existing=currentCart.find(x=>x.itemId===id||x.id===id||x.code===item.code);if(existing){existing.qty=Number(existing.qty||0)+1;}else{currentCart.push({itemId:item.id,id:item.id,code:item.code||"",name:item.nameAr||item.name||"",nameEn:item.nameEn||"",unit:item.unit||"",qty:1});}drawWarehouseCart();drawWarehouseRequestItemsMobile();}
function drawWarehouseCart(){const box=document.getElementById("warehouseCartBox");if(!box)return;if(!currentCart.length){box.innerHTML=`<div class="chef-empty">السلة فارغة</div>`;return;}box.innerHTML=`<div class="warehouse-cart-list">${currentCart.map((i,idx)=>`<div class="warehouse-cart-row"><div><b>${escapeHtml(i.name||"")}</b><small>${escapeHtml(i.unit||"")}</small></div><input type="number" min="1" value="${escapeHtml(i.qty||1)}" onchange="currentCart[${idx}].qty=Number(this.value||1)"><button onclick="currentCart.splice(${idx},1);drawWarehouseCart();"><i class="fa-solid fa-xmark"></i></button></div>`).join("")}</div>`;}
