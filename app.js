...
/* ===== V2.9 Mobile Icons + Brand Polish ===== */
/* تحسين أيقونات وهوية الجوال فقط بدون تغيير منطق التشغيل */

function pc29EnhanceIcons(){
  document.querySelectorAll(
    ".pc21-admin-card-icon,.pc27-task-row>i,.pc26-pdf-grid i,.pcAdminCleanIcon,.pc19-warehouse-kpis i,.pc20-report-icon,.pc27-head button i,.pc21-admin-head button i,.pc18-bottom-nav i"
  ).forEach(el=>{
    el.classList.add("pc29-icon-polished");
  });

  document.querySelectorAll(".pc18-bottom-nav button").forEach(btn=>{
    btn.classList.add("pc29-nav-btn");
  });
}

const pc29OldPageLayout = typeof pageLayout === "function" ? pageLayout : null;
function pageLayout(title,html,backFn){
  if(pc29OldPageLayout){
    pc29OldPageLayout(title,html,backFn);
  }else{
    app.innerHTML = `<main class="app">${html}</main>`;
  }
  setTimeout(pc29EnhanceIcons, 30);
}

document.addEventListener("DOMContentLoaded", ()=>{
  setTimeout(pc29EnhanceIcons, 200);
});
setTimeout(pc29EnhanceIcons, 500);
