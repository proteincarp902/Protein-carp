...
function pc18BottomNav(active="home"){
  return `
    <nav class="pc18-bottom-nav">
      ...
      <button class="${active==="dashboard"?"active":""}" onclick="renderAdminGate()"><i class="fa-solid fa-gauge-high"></i><span>الإدارة</span></button>
  ...
  pageLayout("الإدارة", `
    <section class="pc18-dashboard">
      <div class="pc18-dash-head">
        <button onclick="renderHome()"><i class="fa-solid fa-arrow-right"></i></button>
        <div><span>الإدارة</span><h1>الإدارة</h1><small>${todayDate()}</small></div>
...
/* ===== Mobile Premium V2.0 Admin Gate Fix ===== */

function pc20GoAdmin(){
  if(typeof renderAdminGate === "function"){
...
function pc18BottomNav(active="home"){
  return `
    <nav class="pc18-bottom-nav">
      ...
      <button class="${active==="dashboard" || active==="admin" ? "active":""}" onclick="pc20GoAdmin()">
        <i class="fa-solid fa-gauge-high"></i><span>الإدارة</span>
      </button>
      <button class="${active==="settings"?"active":""}" onclick="renderSettingsGate()">
...
/* ===== Mobile Premium V2.0 Admin Gate Direct Fix ===== */
  ...
  pageLayout("دخول الإدارة", `
    <section class="pc20-admin-gate">
      <div class="pc20-admin-gate-card">
        ...
        <input id="pc20AdminPasswordInput" type="password" placeholder="كلمة مرور الإدارة" onkeydown="if(event.key==='Enter') pc20CheckAdminPassword()">
        <button class="btn btn-main" onclick="pc20CheckAdminPassword()">دخول الإدارة</button>
      </div>
    </section>
    ${typeof pc18BottomNav==="function" ? pc18BottomNav("admin") : ""}
  `,"renderHome()");
  setTimeout(()=>document.getElementById("pc20AdminPasswordInput")?.focus(),80);
}

function pc20CheckAdminPassword(){
  const pass = document.getElementById("pc20AdminPasswordInput")?.value?.trim() || "";
  if(pass !== String(systemSettings.adminPassword)){
    showToast("كلمة المرور غير صحيحة","error");
    return;
  }
  if(typeof renderAdminMobileDashboard === "function" && window.innerWidth <= 900){
    return renderAdminMobileDashboard();
  }
  return pc20RenderAdminSafe();
}

function pc20RenderAdminSafe(){
  if(typeof renderAdminMobileDashboard === "function"){
    return renderAdminMobileDashboard();
  }
  if(typeof renderAdminPDF === "function"){
    return renderAdminPDF();
  }
  pageLayout("الإدارة", `<div class="panel">الإدارة</div>`, "renderHome()");
}

/* نجعل زر الإدارة في الشريط السفلي يدخل للبوابة فقط */
function pc20GoAdmin(){
  return pc20OpenAdminGate();
}
...
function pc18BottomNav(active="home"){
  return `
    <nav class="pc18-bottom-nav">
      ...
      <button class="${active==="dashboard" || active==="admin" ? "active":""}" onclick="pc20OpenAdminGate()">
        <i class="fa-solid fa-gauge-high"></i><span>الإدارة</span>
      </button>
      <button class="${active==="settings"?"active":""}" onclick="renderSettingsGate()">
        <i class="fa-solid fa-gear"></i><span>الإعدادات</span>
...
/* لو أي مكان قديم استدعى renderAdminGate نضمن أنه يفتح البوابة المصححة */
function renderAdminGate(){
  return pc20OpenAdminGate();
}
