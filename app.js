...
/* ===== V2.3 Admin Followup Practical Fix ===== */

function pc23SafeList(name){
...
function pc23DateKey(x){
  if(typeof pc21DateKey === "function") return pc21DateKey(x);
  if(typeof pc17DateKey === "function") return pc17DateKey(x);
  try{
    const ms = typeof getTimeValue === "function" ? getTimeValue(x) : (x.timeMs || x.createdAtMs || 0);
    if(ms){
      const d = new Date(ms);
      return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;
    }
...
function pc23TodayKey(){
  if(typeof pc21TodayKey === "function") return pc21TodayKey();
  const d = new Date();
  return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;
}

function pc23TimeValue(x){
  try{
    if(typeof getTimeValue === "function") return getTimeValue(x);
  }catch(e){}
  return x.timeMs || x.createdAtMs || 0;
}

function pc23ShiftName(shift){
  try{
...
function pc23PendingChefs(){
  const key = pc23TodayKey();
  const prodToday = pc23SafeList("productionLogs").filter(x=>pc23DateKey(x)===key);
  return pc23SafeList("chefs").filter(c=>{
    return !prodToday.some(log =>
      String(log.chefCode || "") === String(c.code || "") ||
      String(log.chefName || "") === String(c.name || "")
    );
...
function pc23MissingCleaning(){
  const key = pc23TodayKey();
  const tasks = pc23SafeList("cleaningTasks");
  const logs = pc23SafeList("cleaningLogs").filter(l=>pc23DateKey(l)===key);
  ...
  shifts.forEach(shift=>{
    ...
    const shiftLogs = logs
      .filter(l=>l.shift===shift)
      .sort((a,b)=>pc23TimeValue(a)-pc23TimeValue(b));
    ...
    if(!latest){
      rows.push({
        group:"مهام ناقصة",
        title:`النظافة - ${pc23ShiftName(shift)}`,
    ...
    (latest.entries || [])
      ...
      .forEach(e=>{
        rows.push({
          group:"مهام ناقصة",
          title:e.nameAr || "مهمة نظافة",
          meta:`النظافة • ${pc23ShiftName(shift)}`,
          icon:"fa-broom"
        });
      });
  });
...
function pc23MissingOperations(){
  const key = pc23TodayKey();
  const tasks = pc23SafeList("operationTasks");
  const logs = pc23SafeList("operationLogs").filter(l=>pc23DateKey(l)===key);
  const rows = [];
  ...
  tasks.forEach(t=>{
    ...
    if(!done){
      rows.push({
        ...
        meta:`التشغيل • ${t.period || "بدون وقت"}`,
        icon:"fa-industry"
      });
    }
  });
...
function pc23FollowupItems(){
  const pending = pc23PendingChefs();
  const missingCleaning = pc23MissingCleaning();
  const missingOps = pc23MissingOperations();
  return [...pending, ...missingCleaning, ...missingOps];
}

function pc23FollowupPanel(){
  const rows = pc23FollowupItems();
  const shown = rows.slice(0,8);
  const more = rows.length - shown.length;

  if(!rows.length){
    return `
      <section class="pc23-follow-panel ok">
        <div class="pc23-follow-head">
          <div>
            <span>يحتاج متابعة</span>
            <h3>لا توجد متابعات حالياً</h3>
          </div>
  ...
  return `
    <section class="pc23-follow-panel">
      <div class="pc23-follow-head">
        <div>
          <span>يحتاج متابعة</span>
          <h3>${rows.length} عنصر يحتاج إجراء</h3>
        </div>
        <i class="fa-solid fa-list-check"></i>
      </div>

      <div class="pc23-follow-list">
        ${shown.map(r=>`
          <div class="pc23-follow-row">
            <i class="fa-solid ${r.icon}"></i>
            <div>
              <b>${escapeHtml(r.title)}</b>
              <small>${escapeHtml(r.meta)} • ${escapeHtml(r.group)}</small>
            </div>
          </div>
        `).join("")}
      </div>

      ${more > 0 ? `<div class="pc23-more-note">+ ${more} عناصر أخرى داخل الأقسام</div>` : ""}
    </section>
  `;
}

function pc23EnsureStyle(){
  if(document.getElementById("pc23AdminStyle")) return;
  const st = document.createElement("style");
  st.id = "pc23AdminStyle";
  st.textContent = `
    .pc21-admin-page{padding-bottom:140px!important}
    .pc21-admin-hero h2{font-size:30px!important}
    .pc21-admin-grid{
      display:grid!important;
      grid-template-columns:repeat(2,minmax(0,1fr))!important;
      gap:12px!important;
    ...
    .pc23-follow-panel{
      ...
      border-radius:28px;
      box-shadow:var(--shadow-soft,0 14px 35px rgba(18,51,37,.10));
      padding:16px;
      margin:14px 0 16px;
    }
    .pc23-follow-panel.ok{
      background:var(--green-soft,#e8f4df);
    }
    .pc23-follow-head{
      display:flex;
      align-items:center;
    ...
    .pc23-follow-head span{
      display:block;
      color:var(--muted,#7b8674);
      font-weight:900;
      font-size:12px;
    }
    .pc23-follow-head h3{
      color:var(--green-dark,#2f5f1f);
      font-size:22px;
      font-weight:900;
      margin:2px 0 0;
    }
    .pc23-follow-head > i{
      ...
      height:52px;
      border-radius:18px;
      background:linear-gradient(135deg,var(--green-dark,#2f5f1f),#113820);
      color:var(--gold,var(--yellow,#f4c542));
      display:flex;
      align-items:center;
    ...
    .pc23-follow-row{
      ...
      padding:10px;
      border-radius:18px;
      background:rgba(239,249,240,.65);
      border:1px solid rgba(67,169,77,.12);
    }
    .pc23-follow-row > i{
      width:38px;
      height:38px;
    ...
    @media(max-width:520px){
      .pc21-admin-grid{grid-template-columns:repeat(2,minmax(0,1fr))!important}
    }
  ...
  app.innerHTML = `
    <main class="app">
      <section class="pc21-admin-page">
        ...
        <section class="pc21-admin-grid" style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;">
          ${pc21AdminSectionCard("warehouse","fa-boxes-stacked","المستودع",s)}
          ${pc21AdminSectionCard("production","fa-chart-line","الإنتاج",s)}
          ${pc21AdminSectionCard("waste","fa-triangle-exclamation","الهدر",s)}
          ${pc21AdminSectionCard("cleaning","fa-broom","النظافة",s)}
