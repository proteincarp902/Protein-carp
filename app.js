...
function renderAdminWarehouse(){
  pageLayout("المستودع", `
    <section class="grid">
      ...
      <div class="card" onclick="pcv1PrintWarehouse()"><div class="icon"><i class="fa-solid fa-file-pdf"></i></div><div class="card-title">طباعة تقرير المستودع</div></div>
    </section>
  `,"renderAdmin()");
}
...
function renderAdminPDF(){
  pageLayout("تصدير PDF", `
    <section class="grid">
      <div class="card" onclick="pcv1PrintWarehouse()"><div class="icon"><i class="fa-solid fa-boxes-stacked"></i></div><div class="card-title">تقرير المستودع</div></div>
      <div class="card" onclick="pcv1PrintProduction()"><div class="icon"><i class="fa-solid fa-chart-line"></i></div><div class="card-title">تقرير الإنتاج</div></div>
      <div class="card" onclick="pcv1PrintWaste()"><div class="icon"><i class="fa-solid fa-trash-can"></i></div><div class="card-title">تقرير التالف والهدر</div></div>
      <div class="card" onclick="pcv1PrintCleaning()"><div class="icon"><i class="fa-solid fa-broom"></i></div><div class="card-title">تقرير النظافة</div></div>
      <div class="card" onclick="pcv1PrintOperations()"><div class="icon"><i class="fa-solid fa-industry"></i></div><div class="card-title">تقرير التشغيل</div></div>
      <div class="card" onclick="pcv1PrintFull()"><div class="icon"><i class="fa-solid fa-file-pdf"></i></div><div class="card-title">التقرير الشامل</div></div>
    </section>
  `,"renderAdmin()");
}
...
/* ===== Protein & Carb PDF V1 - Stable Patch ===== */

function pcv1ItemsText(items){
  return (items || []).map(i=>{
    const name = escapeHtml(i.name || i.productName || "");
    const unit = escapeHtml(i.unit || "");
    const qty = i.qty !== undefined ? `${escapeHtml(i.qty)} ${unit}` : "";
    const issued = i.issuedQty !== undefined ? ` / المصروف: ${escapeHtml(i.issuedQty)} ${unit}` : "";
    return `${name} (${qty}${issued})`;
  }).join("<br>");
}
  ...
  return `
    <style>
      ...
      *{box-sizing:border-box}
      ...
      .pc-report{background:#fff}
      .pc-head{
        text-align:center;
        border-bottom:3px solid #1B4D32;
        padding-bottom:14px;
        margin-bottom:18px;
      }
      .pc-logo{
...
function pcv1Open(title, bodyHtml){
  const win = window.open("", "_blank");
  if(!win){
  ...
  win.document.write(`
    ...
    <body>
      <main class="pc-report">
        <div class="pc-head">
          <img src="assets/logo.png" class="pc-logo" onerror="this.style.display='none'">
          <h1>${escapeHtml(title)}</h1>
          <div class="pc-meta">
            <span>التاريخ: ${escapeHtml(todayDate())}</span>
            <span>وقت الإنشاء: ${escapeHtml(nowText())}</span>
        ...
        ${bodyHtml}
        <div class="pc-foot">تم إنشاء التقرير بواسطة نظام Protein & Carb Operations</div>
      </main>
      <script>
...
function pcv1WarehouseBody(){
  const orders = sortNewest(warehouseOrders.filter(o=>o.status !== "محذوف"));
  const issues = sortNewest(internalIssues);

  return `
    <div class="pc-title">المستودع</div>
    <div class="pc-summary">
      <div class="pc-box">طلبات الشيفات<b>${orders.length}</b></div>
      <div class="pc-box">الصرف الداخلي<b>${issues.length}</b></div>
      <div class="pc-box">إجمالي العمليات<b>${orders.length + issues.length}</b></div>
    </div>

    <div class="pc-subtitle">طلبات الشيفات</div>
    ${orders.length ? `
      <table>
        <thead>
          <tr>
        ...
        <tbody>
          ${orders.map(o=>`
            <tr>
              ...
              <td>${escapeHtml(o.section || "")}</td>
              <td>${escapeHtml(o.status || "")}</td>
              <td>${escapeHtml((typeof getOrderIssueSummary==="function" ? (o.issueStatus || getOrderIssueSummary(o)) : (o.issueStatus || "")))}</td>
              <td>${pcv1ItemsText(o.items)}</td>
              <td>${escapeHtml(o.createdAtText || o.issuedAtText || o.archivedAtText || "")}</td>
            </tr>
          `).join("")}
        </tbody>
      </table>
    ` : `<div class="pc-empty">لا توجد طلبات شيفات</div>`}

    <div class="pc-subtitle">الصرف الداخلي</div>
    ${issues.length ? `
      <table>
        <thead>
          <tr>
    ...
    ` : `<div class="pc-empty">لا يوجد صرف داخلي</div>`}
  `;
}

function pcv1ProductionBody(list){
  list = sortNewest(list || productionLogs);
  return `
    <div class="pc-title">الإنتاج</div>
    ${list.length ? `
      <table>
        <thead><tr><th>الشيف</th><th>القسم</th><th>الأصناف</th><th>ملاحظة</th><th>الوقت</th></tr></thead>
        <tbody>
          ${list.map(log=>`
            <tr>
              <td>${escapeHtml(log.chefName || "")}</td>
              <td>${escapeHtml(log.section || "")}</td>
              <td>${pcv1ItemsText(log.items)}</td>
              <td>${escapeHtml(log.note || "")}</td>
              <td>${escapeHtml(log.createdAtText || "")}</td>
            </tr>
          `).join("")}
        </tbody>
      </table>
    ` : `<div class="pc-empty">لا يوجد إنتاج</div>`}
  `;
}

function pcv1WasteBody(list){
  list = sortNewest(list || wasteLogs);
  return `
    <div class="pc-title">التالف والهدر</div>
    ${list.length ? `
      <table>
        <thead><tr><th>الشيف</th><th>القسم</th><th>الصنف</th><th>الكمية</th><th>السبب</th><th>الوقت</th></tr></thead>
        <tbody>
    ...
    ` : `<div class="pc-empty">لا يوجد تالف أو هدر</div>`}
  `;
}

function pcv1CleaningBody(list){
  list = sortNewest(list || cleaningLogs);
  return `
    <div class="pc-title">النظافة</div>
    ${list.length ? `
      <table>
        <thead><tr><th>الوردية</th><th>المهام</th><th>الوقت</th></tr></thead>
        <tbody>
          ${list.map(log=>`
            <tr>
              <td>${escapeHtml(shiftLabels[log.shift]?.ar || log.shift || "")}</td>
              <td>${(log.entries || []).map(e=>`${escapeHtml(e.nameAr || "")} : ${e.done ? "تم" : "لم يتم"}`).join("<br>")}</td>
              <td>${escapeHtml(log.createdAtText || "")}</td>
            </tr>
          `).join("")}
        </tbody>
      </table>
    ` : `<div class="pc-empty">لا توجد تقارير نظافة</div>`}
  `;
}

function pcv1OperationsBody(list){
  list = sortNewest(list || operationLogs);
  return `
    <div class="pc-title">التشغيل</div>
    ${list.length ? `
      <table>
        <thead><tr><th>المسؤول</th><th>المهمة</th><th>الفترة</th><th>الملاحظة</th><th>الوقت</th></tr></thead>
        <tbody>
    ...
    ` : `<div class="pc-empty">لا توجد تقارير تشغيل</div>`}
  `;
}

function pcv1MarkPrinted(collectionName, list){
  (list || []).filter(x=>x.id).forEach(x=>markPrinted(collectionName, x.id));
}

function pcv1PrintWarehouse(){
  pcv1MarkPrinted("warehouse_orders", warehouseOrders.filter(o=>o.status !== "محذوف"));
  pcv1MarkPrinted("internal_issues", internalIssues);
  pcv1Open("تقرير المستودع", pcv1WarehouseBody());
}

function pcv1PrintProduction(){
  pcv1MarkPrinted("production_logs", productionLogs);
  pcv1Open("تقرير الإنتاج", pcv1ProductionBody(productionLogs));
}

function pcv1PrintWaste(){
  pcv1MarkPrinted("waste_logs", wasteLogs);
  pcv1Open("تقرير التالف والهدر", pcv1WasteBody(wasteLogs));
}

function pcv1PrintCleaning(){
  pcv1MarkPrinted("cleaning_logs", cleaningLogs);
  pcv1Open("تقرير النظافة", pcv1CleaningBody(cleaningLogs));
}

function pcv1PrintOperations(){
  pcv1MarkPrinted("operation_logs", operationLogs);
  pcv1Open("تقرير التشغيل", pcv1OperationsBody(operationLogs));
}

function pcv1PrintFull(){
  const whCount = warehouseOrders.filter(o=>o.status !== "محذوف").length + internalIssues.length;
  const body = `
    <div class="pc-title">ملخص اليوم</div>
    <div class="pc-summary">
      <div class="pc-box">المستودع<b>${whCount}</b></div>
      <div class="pc-box">الإنتاج<b>${productionLogs.length}</b></div>
      <div class="pc-box">التالف والهدر<b>${wasteLogs.length}</b></div>
      <div class="pc-box">النظافة<b>${cleaningLogs.length}</b></div>
      <div class="pc-box">التشغيل<b>${operationLogs.length}</b></div>
      <div class="pc-box">إجمالي العمليات<b>${whCount + productionLogs.length + wasteLogs.length + cleaningLogs.length + operationLogs.length}</b></div>
    </div>
    ${pcv1WarehouseBody()}
    ${pcv1ProductionBody(productionLogs)}
    ${pcv1WasteBody(wasteLogs)}
    ${pcv1CleaningBody(cleaningLogs)}
    ${pcv1OperationsBody(operationLogs)}
  `;
  ...
  pcv1Open("التقرير الشامل", body);
}

/* ربط نهائي آمن لأزرار PDF القديمة والجديدة */
function openPrintReport(title, content){ pcv1Open(title, content); }
function printWarehouseReport(){ pcv1PrintWarehouse(); }
function printAllInternalIssue(){ pcv1PrintWarehouse(); }
function printAllProduction(){ pcv1PrintProduction(); }
function printAllWaste(){ pcv1PrintWaste(); }
function printAllCleaning(){ pcv1PrintCleaning(); }
function printAllOperations(){ pcv1PrintOperations(); }
function printFullDailyReport(){ pcv1PrintFull(); }
