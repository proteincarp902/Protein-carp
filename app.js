...
/* ===== V4.3 Approved Base + Identity UI + Fixes ===== */
/* قاعدة معتمدة + واجهة هوية اللوجو + إصلاحات الإعدادات والمهام */

function pc43Logo(){
  return `<img class="pc43-logo-img" src="assets/logo.png" alt="Protein & Carb" onerror="this.onerror=null;this.src='logo.png';">`;
}

function pc43Counts(){
  try{
    return typeof pc32Counts === "function" ? pc32Counts() : {newWh:0,pending:0};
  }catch(e){
    return {newWh:0,pending:0};
  }
}

function pc43NotificationsCount(){
  try{
    return typeof pc33NotificationRows === "function" ? pc33NotificationRows().length : 0;
  }catch(e){
    return 0;
  }
}

function pc43HomeCard(icon,title,sub,fn,badge=""){
  return `
    <button type="button" class="pc43-card" onclick="${fn}">
      ${badge ? `<em>${badge}</em>` : ""}
      <div class="pc43-icon"><i class="fa-solid ${icon}"></i></div>
      <b>${escapeHtml(title)}</b>
      <span>${escapeHtml(sub)}</span>
...
function pc43FollowCard(c){
  let prodQty = 0;
  let waste = 0;
  try{
    pc32Today(productionLogs || []).forEach(log => (log.items || []).forEach(i => prodQty += Number(i.qty || 0)));
    waste = pc32Today(wasteLogs || []).length;
  }catch(e){}

  const notifyCount = pc43NotificationsCount();

  return `
    <button type="button" class="pc43-follow" onclick="renderAdminGate()">
      <div class="pc43-follow-head">
        <h3>مركز المتابعة</h3>
        <i class="fa-solid fa-chart-line"></i>
      </div>
      <div class="pc43-follow-grid">
        <div><span>طلبات</span><b>${c.newWh || 0}</b></div>
        <div><span>إنتاج</span><b>${prodQty}</b></div>
        <div><span>هدر</span><b>${waste}</b></div>
        <div><span>تنبيه</span><b>${notifyCount}</b></div>
      </div>
    </button>
  `;
}
...
function renderHome(){
  ...
  app.innerHTML = `
    <main class="app pc43-home">
      <section class="pc43-hero">
        <button type="button" class="pc43-bell" onclick="renderNotifications()" aria-label="الإشعارات">
          <i class="fa-regular fa-bell"></i>
          ${notifyCount ? `<em>${notifyCount}</em>` : ""}
        </button>

        <button type="button" class="pc43-date" onclick="renderSettingsGate()" aria-label="الإعدادات">
          <i class="fa-regular fa-calendar-days"></i>
          <span>${todayDate()}</span>
        </button>

        <div class="pc43-logo">${pc43Logo()}</div>
        <h1><span>Protein</span> &amp; Carb</h1>
        <h2>Operations</h2>
        <p>Healthy food</p>

        <div class="pc43-wave"><i></i><b></b></div>
      </section>

      <section class="pc43-title">
        <i></i>
        <h2>الأقسام الرئيسية</h2>
        <i></i>
      </section>

      <section class="pc43-grid">
        ${pc43HomeCard("fa-utensils","الشيفات","إدارة الإنتاج والطلبات","renderChefs()",c.pending ? `${c.pending}` : "")}
        ${pc43HomeCard("fa-boxes-stacked","المستودع","المخزون والطلبات","renderWarehouseGate()",c.newWh ? `${c.newWh}` : "")}
        ${pc43HomeCard("fa-broom","النظافة","مهام النظافة اليومية","renderCleaning()")}
        ${pc43HomeCard("fa-gears","التشغيل","المهام اليومية","renderOperations()")}
      </section>
...
/* إعادة رسم الرئيسية بعد التحميل لو كان الرسم القديم سبق التعريف */
setTimeout(()=>{
  try{
    const oldHome = document.querySelector(".pc32-home,.pc33-home,.pc34-home,.pc35-home,.pc36-home,.pc37-home,.pc38-home,.pc39-home");
    if(oldHome && !document.querySelector(".pc43-home")) renderHome();
  }catch(e){}
},250);

/* الإعدادات تفتح مباشرة */
function renderSettingsGate(){
  try{
...
const pc43OldSettingsShell = typeof settingsShell === "function" ? settingsShell : null;
function settingsShell(active,title,bodyHtml){
  if(pc43OldSettingsShell){
    pc43OldSettingsShell(active,title,bodyHtml);
    try{
      const backBtn = document.querySelector(".pc32-settings-head button:first-child, .settings-head button:first-child, .pc27-head button:first-child");
      if(backBtn && active !== "overview"){
        backBtn.setAttribute("onclick","pc43BackToSettings()");
      }
    }catch(e){}
  }
}

/* تحديث المهام بعد الحفظ */
const pc43OldRefreshViews = typeof refreshViews === "function" ? refreshViews : null;
function refreshViews(){
  if(pc43OldRefreshViews) pc43OldRefreshViews();
  setTimeout(()=>{
    try{
...
/* تنظيف علامة الصح عند تغير الفترة */
function pc43CurrentSlotKey(){
  const d = new Date();
  const h = d.getHours();
...
function pc43ResetDraftsIfNeeded(){
  try{
    const key = pc43CurrentSlotKey();
    const old = localStorage.getItem("pc43_slot_key");
    if(old !== key){
      localStorage.setItem("pc43_slot_key", key);
...
const pc43OldDrawCleaning = typeof pc27DrawCleaningTasks === "function" ? pc27DrawCleaningTasks : null;
function pc27DrawCleaningTasks(){
  pc43ResetDraftsIfNeeded();
  if(pc43OldDrawCleaning) pc43OldDrawCleaning();
}

const pc43OldDrawOperations = typeof pc27DrawOperationTasks === "function" ? pc27DrawOperationTasks : null;
function pc27DrawOperationTasks(){
  pc43ResetDraftsIfNeeded();
  if(pc43OldDrawOperations) pc43OldDrawOperations();
}

const pc43OldSubmitCleaning = typeof pc27SubmitCleaning === "function" ? pc27SubmitCleaning : null;
async function pc27SubmitCleaning(){
  if(!pc43OldSubmitCleaning) return;
  await pc43OldSubmitCleaning();
  try{
    pc27CleaningDraft = {};
...
const pc43OldSubmitOperations = typeof pc27SubmitOperations === "function" ? pc27SubmitOperations : null;
async function pc27SubmitOperations(){
  if(!pc43OldSubmitOperations) return;
  await pc43OldSubmitOperations();
  try{
    pc27OperationsDraft = {};
