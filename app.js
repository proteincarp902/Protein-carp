...
/* ===== Mobile Premium V1.6 Chef Sections + Admin Mobile ===== */

function pcSectionIconClass(sectionName){
  const n = String(sectionName || "");
  if(n.includes("حلا") || n.includes("حلويات")) return "fa-cake-candles";
  if(n.includes("مخبوز") || n.includes("مخبوزات") || n.includes("فرن")) return "fa-bread-slice";
  if(n.includes("منيو") || n.includes("وجبات")) return "fa-book-open";
  if(n.includes("بوفيه") || n.includes("بوفيه")) return "fa-bowl-food";
  if(n.includes("مشوي") || n.includes("شوي")) return "fa-fire-burner";
  if(n.includes("سلطة")) return "fa-leaf";
  return "fa-utensils";
}

function pcSectionChefs(sectionName){
  return (chefs||[]).filter(c=>c.section===sectionName);
}

function pcSectionProductionToday(sectionName){
  const key = typeof pcTodayKey === "function" ? pcTodayKey() : new Date().toISOString().slice(0,10);
  return (productionLogs||[]).filter(log=>{
    const sameSection = String(log.section||"") === String(sectionName||"");
    const sameDay = typeof pcDateKey === "function" ? pcDateKey(log)===key : true;
    return sameSection && sameDay;
  });
}

function pcSectionStatus(sectionName){
  const chefsCount = pcSectionChefs(sectionName).length;
  const logs = pcSectionProductionToday(sectionName);
  const doneNames = new Set(logs.map(x=>String(x.chefName||"")));
  const done = doneNames.size;
  if(!chefsCount) return {text:"لا يوجد شيفات", cls:"muted", done:0, total:0};
  if(done >= chefsCount) return {text:"جاهز", cls:"ready", done, total:chefsCount};
  if(done > 0) return {text:"قيد العمل", cls:"working", done, total:chefsCount};
  return {text:"يحتاج متابعة", cls:"attention", done:0, total:chefsCount};
}

function renderChefs(){
  pageLayout("الشيفات", `
    <section class="pc-chefs-mobile-page">
      <div class="pc-chefs-top">
        <button class="pc-chefs-nav-btn" onclick="renderHome()"><i class="fa-solid fa-arrow-right"></i></button>
        <div>
          <span>Protein & Carb Operations</span>
          <h1>الشيفات</h1>
        </div>
        <button class="pc-chefs-nav-btn" onclick="renderAdminGate()"><i class="fa-solid fa-chart-line"></i></button>
      </div>

      <section class="pc-chefs-banner">
        <div>
          <span>أقسام الإنتاج</span>
          <h2>اختر القسم للدخول</h2>
          <p>تسجيل الإنتاج اليومي ومتابعة الشيفات حسب القسم.</p>
        </div>
      </section>

      <div id="chefSectionsView" class="pc-smart-sections"></div>

      <section class="pc-chefs-info-banner">
        <div class="pc-info-icon"><i class="fa-solid fa-shield-halved"></i></div>
        <div>
          <b>أقسام الإنتاج</b>
          <span>اختر القسم لمتابعة الشيفات وتسجيل الإنتاج اليومي</span>
        </div>
      </section>
    </section>
  `,"renderHome()");
...
function drawChefSectionsView(){
  ...
  if(!chefSections.length){
    box.innerHTML=`<div class="pc-empty">لا توجد أقسام</div>`;
    return;
  }

  box.innerHTML = chefSections.map(s=>{
    const st = pcSectionStatus(s.name);
    return `
      <button class="pc-section-card ${st.cls}" onclick="renderChefCode('${s.name}')">
        <div class="pc-section-icon-wrap">
          <div class="pc-section-icon">
            <i class="fa-solid ${pcSectionIconClass(s.name)}"></i>
          </div>
        </div>
        <div class="pc-section-body">
          <h3>${escapeHtml(s.name||"")}</h3>
          <div class="pc-section-meta">
            <span><i class="fa-solid fa-user-group"></i> ${st.total || pcSectionChefs(s.name).length} شيفات</span>
            <span><i class="fa-solid fa-chart-simple"></i> إنتاج اليوم: ${st.done} / ${st.total}</span>
          </div>
          <div class="pc-section-status ${st.cls}">${st.text}</div>
        </div>
        <div class="pc-section-enter">
          <span>دخول</span>
...
function pcAdminMobileSummary(){
  const key = typeof pcTodayKey === "function" ? pcTodayKey() : new Date().toISOString().slice(0,10);
  const todayProd = (productionLogs||[]).filter(x => typeof pcDateKey === "function" ? pcDateKey(x)===key : true);
  const todayWaste = (wasteLogs||[]).filter(x => typeof pcDateKey === "function" ? pcDateKey(x)===key : true);
  const todayCleaning = (cleaningLogs||[]).filter(x => typeof pcDateKey === "function" ? pcDateKey(x)===key : true);
  const todayOps = (operationLogs||[]).filter(x => typeof pcDateKey === "function" ? pcDateKey(x)===key : true);
  ...
  const pending = (chefs||[]).filter(c=>{
    const done = todayProd.some(log => String(log.chefCode||"")===String(c.code||"") || String(log.chefName||"")===String(c.name||""));
    return !done;
  }).length;
...
function pcTopChefsCompact(limit=4){
  const map = {};
...
function pcTopItemsCompact(limit=4){
  const map = {};
  (productionLogs||[]).forEach(log=>{
    (log.items||[]).forEach(i=>{
...
function pcRecentActivitiesCompact(limit=5){
  const arr = [];
  (productionLogs||[]).forEach(x=>arr.push({t:getTimeValue(x), icon:"fa-chart-line", title:`${x.chefName||"شيف"} رفع إنتاج`, meta:x.createdAtText||""}));
  (warehouseOrders||[]).forEach(x=>arr.push({t:getTimeValue(x), icon:"fa-boxes-stacked", title:`طلب مستودع ${x.status||""}`, meta:`${x.chefName||""} ${x.createdAtText||""}`}));
  (wasteLogs||[]).forEach(x=>arr.push({t:getTimeValue(x), icon:"fa-triangle-exclamation", title:"تسجيل هدر", meta:`${x.chefName||""} ${x.createdAtText||""}`}));
  return arr.sort((a,b)=>b.t-a.t).slice(0,limit);
}

function renderAdminMobileDashboard(){
  const s = pcAdminMobileSummary();
  const topChefs = pcTopChefsCompact();
  const topItems = pcTopItemsCompact();
  const acts = pcRecentActivitiesCompact();

  pageLayout("لوحة التحكم", `
    <section class="pc-admin-mobile-page">
      <div class="pc-admin-mobile-head">
        <button class="pc-chefs-nav-btn" onclick="renderHome()"><i class="fa-solid fa-arrow-right"></i></button>
        <div>
          <span>الإدارة</span>
          <h1>لوحة التحكم</h1>
        ...
        <button class="pc-chefs-nav-btn" onclick="renderAdminPDF()"><i class="fa-solid fa-file-pdf"></i></button>
      </div>

      <section class="pc-admin-hero">
        <span>ملخص التشغيل</span>
        <h2>وضع اليوم خلال 5 ثواني</h2>
      </section>

      <section class="pc-admin-kpis">
      ...
      <section class="pc-admin-mobile-grid">
        <div class="pc-admin-mobile-card">
          <div class="pc-panel-head"><h3>أفضل الشيفات</h3></div>
          ${topChefs.length ? topChefs.map((x,i)=>`<div class="pc-mini-rank"><span>${i+1}</span><b>${escapeHtml(x.name)}</b><strong>${x.qty}</strong></div>`).join("") : `<div class="pc-empty">لا توجد بيانات</div>`}
        </div>

        <div class="pc-admin-mobile-card">
          <div class="pc-panel-head"><h3>أكثر الأصناف</h3></div>
          ${topItems.length ? topItems.map((x,i)=>`<div class="pc-mini-rank"><span>${i+1}</span><b>${escapeHtml(x.name)}</b><strong>${x.qty}</strong></div>`).join("") : `<div class="pc-empty">لا توجد بيانات</div>`}
        </div>

        <div class="pc-admin-mobile-card wide">
          <div class="pc-panel-head"><h3>آخر الأنشطة</h3></div>
          ${acts.length ? acts.map(a=>`<div class="pc-mini-activity"><i class="fa-solid ${a.icon}"></i><div><b>${escapeHtml(a.title)}</b><small>${escapeHtml(a.meta)}</small></div></div>`).join("") : `<div class="pc-empty">لا توجد أنشطة</div>`}
        </div>
      </section>

      <section class="pc-admin-shortcuts">
...
const pcOldRenderAdmin = typeof renderAdmin === "function" ? renderAdmin : null;
function renderAdmin(){
  if(window.innerWidth <= 760){
    return renderAdminMobileDashboard();
  }
  if(pcOldRenderAdmin) return pcOldRenderAdmin();
  return renderAdminMobileDashboard();
}
