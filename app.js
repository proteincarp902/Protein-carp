...
/* ===== Mobile Premium V2.0 Reports + Boards ===== */
...
function pc20Today(list){
  const k=pc20DayKey();
  return (list||[]).filter(x=>pc20DateKey(x)===k);
}
function pc20Qty(list){
  let n=0;
  (list||[]).forEach(log=>(log.items||[]).forEach(i=>n+=Number(i.qty||0)));
  return n;
}
function pc20Stats(){
  const prod=pc20Today(productionLogs), wh=pc20Today(warehouseOrders), waste=pc20Today(wasteLogs), internal=pc20Today(internalIssues), cleaning=pc20Today(cleaningLogs), ops=pc20Today(operationLogs);
  const pending=(chefs||[]).filter(c=>!prod.some(log=>String(log.chefCode||"")===String(c.code||"")||String(log.chefName||"")===String(c.name||""))).length;
  return {prod, wh, waste, internal, cleaning, ops, qty:pc20Qty(prod), pending};
}
function pc20Print(title,html){
  if(typeof openPrintReport==="function") return openPrintReport(title,html);
  const w=window.open("","_blank");
  w.document.write(`<html dir="rtl"><head><meta charset="UTF-8"><title>${title}</title>${typeof getReportCss==="function"?getReportCss():""}</head><body>${html}</body></html>`);
  w.document.close(); w.print();
}
function pc20PrintDaily(){
  pc20Print("التقرير اليومي الشامل",`${typeof buildWarehouseReport==="function"?buildWarehouseReport():""}${typeof buildProductionReport==="function"?buildProductionReport(pc20Today(productionLogs)):""}${typeof buildWasteReport==="function"?buildWasteReport(pc20Today(wasteLogs)):""}${typeof buildCleaningReport==="function"?buildCleaningReport(pc20Today(cleaningLogs)):""}${typeof buildOperationsReport==="function"?buildOperationsReport(pc20Today(operationLogs)):""}`);
}
function pc20PrintWarehouse(){pc20Print("تقرير المستودع",typeof buildWarehouseReport==="function"?buildWarehouseReport():"");}
function pc20PrintProduction(){pc20Print("تقرير الإنتاج",typeof buildProductionReport==="function"?buildProductionReport(pc20Today(productionLogs)):"");}
function pc20PrintWaste(){pc20Print("تقرير الهدر والتالف",typeof buildWasteReport==="function"?buildWasteReport(pc20Today(wasteLogs)):"");}
function pc20PrintCleaning(){pc20Print("تقرير النظافة",typeof buildCleaningReport==="function"?buildCleaningReport(pc20Today(cleaningLogs)):"");}
function pc20PrintOps(){pc20Print("تقرير التشغيل",typeof buildOperationsReport==="function"?buildOperationsReport(pc20Today(operationLogs)):"");}

function pc20ReportCard(icon,title,desc,fn,badge){
  return `<button class="pc20-report-card" onclick="${fn}"><div class="pc20-report-icon"><i class="fa-solid ${icon}"></i></div><div><b>${title}</b><span>${desc}</span></div><em>${badge}</em></button>`;
}

function renderAdminPDF(){
  const s=pc20Stats();
  pageLayout("التقارير",`
    <section class="pc20-page">
      <div class="pc20-head">
        <button onclick="renderAdmin()"><i class="fa-solid fa-arrow-right"></i></button>
        <div><span>الإدارة</span><h1>التقارير</h1><small>${todayDate()}</small></div>
        <button onclick="pc20PrintDaily()"><i class="fa-solid fa-print"></i></button>
      </div>

      <section class="pc20-hero"><span>Premium Reports</span><h2>تقارير جاهزة للطباعة</h2><p>عرض واضح بدل اللوحات البدائية.</p></section>

      <section class="pc20-summary">
        <div><span>إنتاج اليوم</span><b>${s.qty}</b><small>${s.prod.length} عمليات</small></div>
        <div><span>طلبات المستودع</span><b>${s.wh.length}</b><small>طلب</small></div>
        <div><span>الهدر</span><b>${s.waste.length}</b><small>عملية</small></div>
        <div><span>إنتاج معلق</span><b>${s.pending}</b><small>شيف</small></div>
      </section>

      <section class="pc20-board">
        <div class="${s.pending?'warn':'ok'}"><i class="fa-solid ${s.pending?'fa-clock':'fa-circle-check'}"></i><span>المتابعة</span><b>${s.pending?s.pending+' معلق':'مكتمل'}</b></div>
        <div><i class="fa-solid fa-arrow-right-arrow-left"></i><span>صرف داخلي</span><b>${s.internal.length}</b></div>
        <div><i class="fa-solid fa-broom"></i><span>النظافة</span><b>${s.cleaning.length}</b></div>
      </section>

      <section class="pc20-list">
        ${pc20ReportCard("fa-file-lines","التقرير اليومي الشامل","كل العمليات اليومية في تقرير واحد","pc20PrintDaily()","شامل")}
        ${pc20ReportCard("fa-boxes-stacked","تقرير المستودع","طلبات الشيفات والصرف الداخلي","pc20PrintWarehouse()",s.wh.length)}
        ${pc20ReportCard("fa-chart-line","تقرير الإنتاج","إنتاج الشيفات حسب اليوم","pc20PrintProduction()",s.prod.length)}
        ${pc20ReportCard("fa-triangle-exclamation","تقرير الهدر والتالف","كل عمليات الهدر المسجلة","pc20PrintWaste()",s.waste.length)}
        ${pc20ReportCard("fa-broom","تقرير النظافة","متابعة مهام النظافة","pc20PrintCleaning()",s.cleaning.length)}
        ${pc20ReportCard("fa-industry","تقرير التشغيل","مهام التشغيل اليومية","pc20PrintOps()",s.ops.length)}
      </section>
    </section>
    ${typeof pc18BottomNav==="function"?pc18BottomNav("dashboard"):""}
  `,"renderAdmin()");
}

const pc20OldMobileDash=typeof renderAdminMobileDashboard==="function"?renderAdminMobileDashboard:null;
function renderAdminMobileDashboard(){
  if(pc20OldMobileDash){
    pc20OldMobileDash();
    setTimeout(()=>{
      const dash=document.querySelector(".pc18-dashboard");
      if(dash && !document.querySelector(".pc20-dash-board")){
        const s=pc20Stats();
        const el=document.createElement("section");
        el.className="pc20-dash-board";
        el.innerHTML=`<div class="${s.pending?'warn':'ok'}"><i class="fa-solid ${s.pending?'fa-clock':'fa-circle-check'}"></i><span>إنتاج معلق</span><b>${s.pending}</b></div><div><i class="fa-solid fa-file-lines"></i><span>تقارير اليوم</span><b>${s.prod.length+s.wh.length+s.waste.length}</b></div>`;
        const hero=dash.querySelector(".pc18-dash-hero");
...
/* ===== ADMIN CLICK HARD FIX ===== */

function adminHardTodayKey(){
  const d = new Date();
  return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;
}

function adminHardDateKey(x){
  try{
    if(typeof pc17DateKey === "function") return pc17DateKey(x);
...
function adminHardStats(){
  const key = adminHardTodayKey();
  const prod = (window.productionLogs || productionLogs || []).filter(x=>adminHardDateKey(x)===key);
  const wh = (window.warehouseOrders || warehouseOrders || []).filter(x=>adminHardDateKey(x)===key);
  const waste = (window.wasteLogs || wasteLogs || []).filter(x=>adminHardDateKey(x)===key);
  const cleaning = (window.cleaningLogs || cleaningLogs || []).filter(x=>adminHardDateKey(x)===key);
  const ops = (window.operationLogs || operationLogs || []).filter(x=>adminHardDateKey(x)===key);

  let qty = 0;
  prod.forEach(log => (log.items || []).forEach(i => qty += Number(i.qty || 0)));

  const chefList = (window.chefs || chefs || []);
  const pending = chefList.filter(c=>{
    return !prod.some(log =>
  ...
  return {
    newWh:(window.warehouseOrders || warehouseOrders || []).filter(o=>o.status==="جديد").length,
    qty,
    waste:waste.length,
...
function adminHardOpen(){
  const s = adminHardStats();
  ...
  app.innerHTML = `
    <main class="app">
      <section class="pc18-dashboard">
        <div class="pc18-dash-head">
          <button id="adminHardBackBtn" type="button"><i class="fa-solid fa-arrow-right"></i></button>
          <div>
            <span>الإدارة</span>
            <h1>الإدارة</h1>
            <small>${typeof todayDate === "function" ? todayDate() : ""}</small>
          </div>
          <button id="adminHardReportsBtn" type="button"><i class="fa-solid fa-file-pdf"></i></button>
        </div>

        <section class="pc18-dash-hero">
          <span>ملخص التشغيل</span>
          <h2>وضع اليوم</h2>
          <p>أهم مؤشرات المطعم في شاشة واحدة.</p>
        </section>

        <section class="pc18-kpi-grid">
          <div><span>طلبات المستودع</span><b>${s.newWh}</b><small>جديد</small></div>
          <div><span>إنتاج اليوم</span><b>${s.qty}</b><small>كمية</small></div>
          <div><span>الهدر</span><b>${s.waste}</b><small>عملية</small></div>
          <div><span>النظافة</span><b>${s.cleaning}</b><small>تقرير</small></div>
          <div><span>التشغيل</span><b>${s.ops}</b><small>تقرير</small></div>
          <div class="${s.pending ? "warn" : ""}"><span>إنتاج معلق</span><b>${s.pending}</b><small>شيف</small></div>
        ...
        <section class="pc18-dash-shortcuts">
          <button type="button" id="adminHardWarehouseBtn"><i class="fa-solid fa-boxes-stacked"></i><span>المستودع</span></button>
          <button type="button" id="adminHardProductionBtn"><i class="fa-solid fa-chart-line"></i><span>الإنتاج</span></button>
          <button type="button" id="adminHardWasteBtn"><i class="fa-solid fa-triangle-exclamation"></i><span>الهدر</span></button>
          <button type="button" id="adminHardPdfBtn"><i class="fa-solid fa-file-pdf"></i><span>التقارير</span></button>
        </section>
      </section>
    </main>
    ${bottom}
  ...
  setTimeout(()=>{
    document.getElementById("adminHardBackBtn")?.addEventListener("click", ()=>renderHome());
    document.getElementById("adminHardReportsBtn")?.addEventListener("click", ()=>{ if(typeof renderAdminPDF==="function") renderAdminPDF(); });
    document.getElementById("adminHardWarehouseBtn")?.addEventListener("click", ()=>{ if(typeof renderAdminWarehouse==="function") renderAdminWarehouse(); });
    document.getElementById("adminHardProductionBtn")?.addEventListener("click", ()=>{ if(typeof renderAdminProduction==="function") renderAdminProduction(); });
    document.getElementById("adminHardWasteBtn")?.addEventListener("click", ()=>{ if(typeof renderAdminWaste==="function") renderAdminWaste(); });
    document.getElementById("adminHardPdfBtn")?.addEventListener("click", ()=>{ if(typeof renderAdminPDF==="function") renderAdminPDF(); });
  }, 0);
}

function adminHardCheck(){
  const input = document.getElementById("adminHardPassword");
  const pass = input ? input.value.trim() : "";
  const correct = String((systemSettings && systemSettings.adminPassword) ? systemSettings.adminPassword : "0000");
...
function renderAdminGate(){
  ...
  app.innerHTML = `
    <main class="app">
      <section class="pc20-admin-gate">
        <div class="pc20-admin-gate-card">
          <div class="pc20-admin-gate-icon"><i class="fa-solid fa-shield-halved"></i></div>
          <h2>الإدارة</h2>
          <p>أدخل كلمة مرور الإدارة للمتابعة</p>
          <input id="adminHardPassword" type="password" placeholder="كلمة مرور الإدارة" autocomplete="current-password">
          <button id="adminHardLoginBtn" type="button" class="btn btn-main">دخول الإدارة</button>
        </div>
      </section>
    </main>
    ${bottom}
  ...
  setTimeout(()=>{
    const input = document.getElementById("adminHardPassword");
    const btn = document.getElementById("adminHardLoginBtn");
    input?.focus();
    btn?.addEventListener("click", adminHardCheck);
    input?.addEventListener("keydown", e=>{
      if(e.key === "Enter") adminHardCheck();
    });
  }, 0);
}
...
function pc20CheckAdminPassword(){
  adminHardCheck();
}

function pc20OpenAdminGate(){
  renderAdminGate();
}

function pc20GoAdmin(){
  renderAdminGate();
}
...
window.pc20OpenAdminGate = pc20OpenAdminGate;
window.pc20GoAdmin = pc20GoAdmin;
window.pc20CheckAdminPassword = pc20CheckAdminPassword;
window.adminHardCheck = adminHardCheck;
window.adminHardOpen = adminHardOpen;


/* ===== ADMIN SECTIONS COMPLETE FIX ===== */

function pcAdminSectionCard(icon,title,desc,fn,badge){
  return `
    <button type="button" class="pcAdminSectionCard" onclick="${fn}">
...
function pcFinalOpenAdmin(){
  const s = adminHardStats ? adminHardStats() : {newWh:0,qty:0,waste:0,cleaning:0,ops:0,pending:0};
  const bottom = (typeof pc18BottomNav === "function") ? pc18BottomNav("admin") : "";

  app.innerHTML = `
    <main class="app">
      <section class="pc18-dashboard pcAdminFull">
        <div class="pc18-dash-head">
          <button id="adminFullBackBtn" type="button"><i class="fa-solid fa-arrow-right"></i></button>
          <div>
            <span>الإدارة</span>
            <h1>الإدارة</h1>
            <small>${typeof todayDate === "function" ? todayDate() : ""}</small>
          </div>
          <button id="adminFullReportsTopBtn" type="button"><i class="fa-solid fa-file-pdf"></i></button>
        </div>

        <section class="pc18-dash-hero">
          <span>ملخص التشغيل</span>
          <h2>لوحة الإدارة</h2>
          <p>متابعة الإنتاج والمستودع والهدر والنظافة والتشغيل من مكان واحد.</p>
        </section>

        <section class="pc18-kpi-grid">
          <div><span>طلبات المستودع</span><b>${s.newWh || 0}</b><small>جديد</small></div>
          <div><span>إنتاج اليوم</span><b>${s.qty || 0}</b><small>كمية</small></div>
          <div><span>الهدر</span><b>${s.waste || 0}</b><small>عملية</small></div>
          <div><span>النظافة</span><b>${s.cleaning || 0}</b><small>تقرير</small></div>
          <div><span>التشغيل</span><b>${s.ops || 0}</b><small>تقرير</small></div>
          <div class="${s.pending ? "warn" : ""}"><span>إنتاج معلق</span><b>${s.pending || 0}</b><small>شيف</small></div>
        </section>

        <section class="pcAdminSectionsGrid">
          ${pcAdminSectionCard("fa-boxes-stacked","المستودع","طلبات الشيفات وحالات الصرف","renderAdminWarehouse()",s.newWh || "")}
          ${pcAdminSectionCard("fa-chart-line","الإنتاج","إنتاج الشيفات اليومي","renderAdminProduction()",s.qty || "")}
          ${pcAdminSectionCard("fa-triangle-exclamation","الهدر والتالف","متابعة الهدر المسجل","renderAdminWaste()",s.waste || "")}
          ${pcAdminSectionCard("fa-arrow-right-arrow-left","الصرف الداخلي","صرف العمال والجهات الداخلية","renderAdminInternalIssues ? renderAdminInternalIssues() : renderInternalIssuesAdminFallback()","")}
          ${pcAdminSectionCard("fa-broom","النظافة","تقارير ومهام النظافة","renderAdminCleaning ? renderAdminCleaning() : renderCleaning()","")}
          ${pcAdminSectionCard("fa-industry","التشغيل","مهام التشغيل اليومية","renderAdminOperations ? renderAdminOperations() : renderOperations()","")}
          ${pcAdminSectionCard("fa-file-pdf","التقارير PDF","طباعة وحفظ تقارير الإدارة","renderAdminPDF()","PDF")}
          ${pcAdminSectionCard("fa-box-archive","الأرشيف","متابعة الأيام السابقة","renderAdminArchive ? renderAdminArchive() : showToast('الأرشيف غير مفعل بعد')","")}
        </section>
      </section>
    </main>
    ${bottom}
  ...
  setTimeout(()=>{
    document.getElementById("adminFullBackBtn")?.addEventListener("click", ()=>renderHome());
    document.getElementById("adminFullReportsTopBtn")?.addEventListener("click", ()=>{ if(typeof renderAdminPDF==="function") renderAdminPDF(); });
  }, 0);
}

function renderInternalIssuesAdminFallback(){
  pageLayout("الصرف الداخلي", `
    <section class="pc18-dashboard">
      <div class="pc18-dash-head">
        <button onclick="pcFinalOpenAdmin()"><i class="fa-solid fa-arrow-right"></i></button>
        <div><span>الإدارة</span><h1>الصرف الداخلي</h1><small>${todayDate()}</small></div>
        <button onclick="renderAdminPDF()"><i class="fa-solid fa-file-pdf"></i></button>
      </div>
      <section class="pc18-dash-card">
        <h3>عمليات الصرف الداخلي</h3>
        ${(internalIssues||[]).length ? (internalIssues||[]).slice(0,50).map(x=>`
          <div class="pc18-activity">
            <i class="fa-solid fa-arrow-right-arrow-left"></i>
            <div>
              <b>${escapeHtml(x.destination || "جهة غير محددة")}</b>
              <small>${escapeHtml(x.staff || "")} - ${escapeHtml(x.createdAtText || "")}</small>
            </div>
          </div>
        `).join("") : `<div class="pc17-empty">لا توجد عمليات صرف داخلي</div>`}
      </section>
    </section>
...
function renderAdmin(){
  return pcFinalOpenAdmin();
}


/* ===== ADMIN CARDS CLEAN PREMIUM FIX ===== */

function pcAdminCleanInjectStyle(){
  if(document.getElementById("pcAdminCleanStyle")) return;
  const st = document.createElement("style");
  st.id = "pcAdminCleanStyle";
  st.textContent = `
    .pcAdminCleanFull{padding-bottom:110px}
    .pcAdminCleanGrid{
      display:grid;
    ...
    .pcAdminCleanIcon{
      ...
      height:62px;
      border-radius:50%;
      background:linear-gradient(135deg,var(--green-dark),#113820);
      color:var(--gold);
      border:3px solid rgba(244,197,66,.72);
      display:flex;
      align-items:center;
      justify-content:center;
      font-size:25px;
      box-shadow:0 12px 24px rgba(18,51,37,.14);
    }
    .pcAdminCleanCard b{
      position:relative;
      z-index:1;
    ...
    .pcAdminCleanCard.warn em{background:#FFF6D6;color:#8A6500}
    .pcAdminCleanCard.danger em{background:#FFF1F1;color:#B91C1C}
    @media(max-width:380px){
      .pcAdminCleanGrid{grid-template-columns:1fr}
    }
  `;
  document.head.appendChild(st);
}

function pcAdminCleanCard(icon,title,metric,fn,badge="",state=""){
  return `
    <button type="button" class="pcAdminCleanCard ${state}" onclick="${fn}">
      <div class="pcAdminCleanIcon"><i class="fa-solid ${icon}"></i></div>
      <b>${title}</b>
      <span>${metric}</span>
...
function pcAdminCleanStats(){
  const s = typeof adminHardStats === "function" ? adminHardStats() : (typeof pcFinalAdminStats === "function" ? pcFinalAdminStats() : {});
  const internalCount = (internalIssues||[]).length;
  const archiveCount = (dailyArchives||archives||[]).length || "";
  return {
    newWh:s.newWh || 0,
    qty:s.qty || 0,
    waste:s.waste || 0,
    cleaning:s.cleaning || 0,
    ops:s.ops || 0,
    pending:s.pending || 0,
...
function pcFinalOpenAdmin(){
  pcAdminCleanInjectStyle();
  const s = pcAdminCleanStats();
  const bottom = (typeof pc18BottomNav === "function") ? pc18BottomNav("admin") : "";

  app.innerHTML = `
    <main class="app">
      <section class="pc18-dashboard pcAdminCleanFull">
        <div class="pc18-dash-head">
          <button id="adminCleanBackBtn" type="button"><i class="fa-solid fa-arrow-right"></i></button>
          <div>
            <span>الإدارة</span>
            <h1>الإدارة</h1>
            <small>${typeof todayDate === "function" ? todayDate() : ""}</small>
          </div>
          <button id="adminCleanReportsTopBtn" type="button"><i class="fa-solid fa-file-pdf"></i></button>
        </div>

        <section class="pc18-dash-hero">
          <span>ملخص التشغيل</span>
          <h2>لوحة الإدارة</h2>
          <p>متابعة سريعة لكل الأقسام.</p>
        </section>
        ...
        <section class="pcAdminCleanGrid">
          ${pcAdminCleanCard("fa-boxes-stacked","المستودع",`${s.newWh} جديد`,"renderAdminWarehouse()",s.newWh || "",s.newWh ? "warn" : "")}
          ${pcAdminCleanCard("fa-chart-line","الإنتاج",`${s.qty} اليوم`,"renderAdminProduction()",s.pending ? `${s.pending} معلق` : "✓",s.pending ? "warn" : "")}
          ${pcAdminCleanCard("fa-triangle-exclamation","الهدر",`${s.waste} اليوم`,"renderAdminWaste()",s.waste || "",s.waste ? "danger" : "")}
          ${pcAdminCleanCard("fa-arrow-right-arrow-left","الصرف الداخلي",`${s.internal} عمليات`,"typeof renderAdminInternalIssues==='function' ? renderAdminInternalIssues() : renderInternalIssuesAdminFallback()",s.internal || "")}
          ${pcAdminCleanCard("fa-broom","النظافة",`${s.cleaning} تقارير`,"typeof renderAdminCleaning==='function' ? renderAdminCleaning() : renderCleaning()",s.cleaning || "")}
          ${pcAdminCleanCard("fa-industry","التشغيل",`${s.ops} تقارير`,"typeof renderAdminOperations==='function' ? renderAdminOperations() : renderOperations()",s.ops || "")}
          ${pcAdminCleanCard("fa-file-pdf","التقارير","PDF يومي","renderAdminPDF()","PDF")}
          ${pcAdminCleanCard("fa-box-archive","الأرشيف",s.archive ? `${s.archive} أيام` : "أيام سابقة","typeof renderAdminArchive==='function' ? renderAdminArchive() : showToast('الأرشيف غير مفعل بعد')",s.archive || "")}
        </section>
      </section>
    </main>
    ${bottom}
  ...
  setTimeout(()=>{
    document.getElementById("adminCleanBackBtn")?.addEventListener("click", ()=>renderHome());
    document.getElementById("adminCleanReportsTopBtn")?.addEventListener("click", ()=>{ if(typeof renderAdminPDF==="function") renderAdminPDF(); });
  }, 0);
}
...
/* ===== ADMIN WAREHOUSE INTERNAL MERGE FIX ===== */
    ...
    <main class="app">
      <section class="pc18-dashboard pcAdminCleanFull">
        ...
        <section class="pc18-dash-hero">
          <span>ملخص التشغيل</span>
          <h2>لوحة الإدارة</h2>
          <p>نظرة سريعة للأقسام التي تحتاج متابعة.</p>
        </section>
        ...
        <section class="pcAdminCleanGrid">
          ${pcAdminCleanCard("fa-boxes-stacked","المستودع",`${s.newWh} جديد • ${s.internal} داخلي`,"renderAdminWarehouse()",s.newWh || s.internal || "",(s.newWh || s.internal) ? "warn" : "")}
          ${pcAdminCleanCard("fa-chart-line","الإنتاج",`${s.qty} اليوم`,"renderAdminProduction()",s.pending ? `${s.pending} معلق` : "✓",s.pending ? "warn" : "")}
          ${pcAdminCleanCard("fa-triangle-exclamation","الهدر",`${s.waste} اليوم`,"renderAdminWaste()",s.waste || "",s.waste ? "danger" : "")}
          ${pcAdminCleanCard("fa-broom","النظافة",`${s.cleaning} تقارير`,"typeof renderAdminCleaning==='function' ? renderAdminCleaning() : renderCleaning()",s.cleaning || "")}
          ${pcAdminCleanCard("fa-industry","التشغيل",`${s.ops} تقارير`,"typeof renderAdminOperations==='function' ? renderAdminOperations() : renderOperations()",s.ops || "")}
