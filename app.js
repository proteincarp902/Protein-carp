...
/* ===== V3.1 Mobile App Redesign ===== */
/* تقسيم وشكل تطبيق جوال — يحافظ على منطق V2.8 */

function pc31TodayKey(){
  const d = new Date();
  return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;
}

function pc31DateKey(x){
  try{
...
function pc31Today(list){
  const key = pc31TodayKey();
  return (list||[]).filter(x=>pc31DateKey(x)===key);
}

function pc31Stats(){
  let prodQty = 0;
  pc31Today(productionLogs||[]).forEach(log=>(log.items||[]).forEach(i=>prodQty += Number(i.qty||0)));
  const newWh = (warehouseOrders||[]).filter(o=>o.status==="جديد" || !o.status).length;
  const pending = (chefs||[]).filter(c=>!pc31Today(productionLogs||[]).some(log=>
    String(log.chefCode||"")===String(c.code||"") || String(log.chefName||"")===String(c.name||"")
  )).length;

  let cleaningMissing = 0;
  try{
    if(typeof pc27CleaningStats === "function") cleaningMissing = pc27CleaningStats().missing || 0;
  }catch(e){}

  let opsMissing = 0;
  try{
    if(typeof pc27OperationStats === "function") opsMissing = pc27OperationStats().missing || 0;
  }catch(e){}

  const waste = pc31Today(wasteLogs||[]).length;
  const internal = pc31Today(internalIssues||[]).length;
  return {prodQty,newWh,pending,waste,cleaningMissing,opsMissing,internal};
}
...
function pc31StatusText(v, goodText, badText){
  return v ? badText : goodText;
}

function pc31HomeTile(icon,title,sub,meta,fn,wide=""){
  return `
    <button type="button" class="pc31-tile ${wide}" onclick="${fn}">
      <div class="pc31-tile-icon"><i class="fa-solid ${icon}"></i></div>
      <div class="pc31-tile-text">
...
function renderHome(){
  ...
  app.innerHTML = `
    <main class="app pc31-app pc31-home">
      <section class="pc31-home-head">
        <button type="button" onclick="renderSettingsGate()"><i class="fa-solid fa-bars"></i></button>
        <div class="pc31-brand">
          <div class="pc31-brand-logo">${pc31Logo()}</div>
          <span>مرحباً بك في نظام</span>
          <h1>Protein & Carb Operations</h1>
          <small>${todayDate()}</small>
        </div>
        <button type="button" onclick="renderAdminGate()"><i class="fa-regular fa-bell"></i></button>
      </section>

      <section class="pc31-hero-card">
        <div>
          <span>تشغيل اليوم</span>
          <h2>${s.newWh || s.pending || s.cleaningMissing || s.opsMissing ? "يحتاج متابعة" : "جاهز للعمل"}</h2>
          <p>الأقسام المهمة في شاشة واحدة.</p>
        </div>
        <div class="pc31-hero-icon"><i class="fa-solid fa-clipboard-check"></i></div>
      </section>

      <section class="pc31-mini-grid">
        <div><span>طلبات</span><b>${s.newWh}</b><small>جديدة</small></div>
        <div><span>إنتاج</span><b>${s.pending}</b><small>معلق</small></div>
        <div><span>مهام</span><b>${s.cleaningMissing + s.opsMissing}</b><small>ناقصة</small></div>
        <div><span>هدر</span><b>${s.waste}</b><small>اليوم</small></div>
      </section>

      <section class="pc31-section-title">
        <h3>الأقسام الرئيسية</h3>
        <span>اختر القسم المطلوب</span>
      </section>

      <section class="pc31-tiles">
        ${pc31HomeTile("fa-utensils","الشيفات","إنتاج وطلبات",`${s.pending} معلق`,"renderChefs()")}
        ${pc31HomeTile("fa-boxes-stacked","المستودع","طلبات وصرف",`${s.newWh} جديد`,"renderWarehouseGate()")}
        ${pc31HomeTile("fa-broom","النظافة","متابعة الورديات",`${s.cleaningMissing} ناقص`,"renderCleaning()")}
        ${pc31HomeTile("fa-gear","التشغيل","فتح وإغلاق",`${s.opsMissing} ناقص`,"renderOperations()")}
        ${pc31HomeTile("fa-gauge-high","الإدارة","متابعة وتقارير","مركز المتابعة","renderAdminGate()","wide")}
        ${pc31HomeTile("fa-file-pdf","التقارير","PDF وأرشيف","طباعة","renderAdminPDF()","wide")}
      </section>
    </main>
...
/* شيفات: نفس المنطق، تقسيم أوضح */
function renderChefs(){
  pcActiveChefSectionName = pcActiveChefSectionName || (chefSections[0]?.name || "");
  pageLayout("الشيفات", `
    <section class="pc31-page pc31-chefs">
      <section class="pc31-screen-head">
        <button type="button" onclick="renderHome()"><i class="fa-solid fa-arrow-right"></i></button>
        <div>
          <span>Production Control</span>
          <h1>الشيفات</h1>
          <small>إنتاج، هدر، وطلبات يومية</small>
        </div>
        <button type="button" onclick="renderAdminGate()"><i class="fa-solid fa-gauge-high"></i></button>
      </section>
      ...
      <section class="pc31-info-card">
        <i class="fa-solid fa-circle-info"></i>
        <div>
          <b>أقسام الإنتاج</b>
          <span>اختر القسم ثم ادخل بكود الشيف لتسجيل الإنتاج بسرعة.</span>
        </div>
      </section>
    </section>
    ${typeof pc18BottomNav === "function" ? pc18BottomNav("chefs") : ""}
  `,"renderHome()");
  drawChefSectionsView();
}

/* مستودع: تقليل التكرار وجعل الواجهة مثل تطبيق */
function renderWarehouse(){
  pc19OpenWarehouseOrderId = null;
  const c = pc19WarehouseCounts();

  pageLayout("المستودع", `
    <section class="pc31-page pc31-warehouse">
      <section class="pc31-screen-head">
        <button type="button" onclick="renderHome()"><i class="fa-solid fa-arrow-right"></i></button>
        <div>
          <span>Warehouse Control</span>
          <h1>المستودع</h1>
          <small>طلبات وصرف اليوم</small>
        </div>
        <button type="button" onclick="renderInternalIssue()"><i class="fa-solid fa-arrow-right-arrow-left"></i></button>
      </section>

      <section class="pc31-warehouse-actions">
        <button type="button" onclick="renderInternalIssue()">
          <i class="fa-solid fa-arrow-right-arrow-left"></i>
          <b>صرف داخلي</b>
          <small>نقل بين الأقسام</small>
        </button>
        <button type="button" onclick="pc19WarehouseTab='new';drawWarehouseOrders()">
          <i class="fa-solid fa-file-circle-plus"></i>
          <b>طلبات جديدة</b>
          <small>${c.newOrders} طلب</small>
        </button>
      </section>

      <section class="pc31-mini-grid pc31-warehouse-grid">
        <div><span>جديدة</span><b>${c.newOrders}</b><small>طلبات</small></div>
        <div><span>قيد الصرف</span><b>${c.inProgress}</b><small>متابعة</small></div>
        <div><span>تم الصرف</span><b>${c.doneToday}</b><small>مكتمل</small></div>
        <div><span>داخلي</span><b>${c.internal}</b><small>صرف</small></div>
      </section>

      <section class="pc19-tabs pc31-tabs">
        <button class="${pc19WarehouseTab==="new"?"active":""}" onclick="pc19WarehouseTab='new';drawWarehouseOrders()">جديد</button>
        <button class="${pc19WarehouseTab==="progress"?"active":""}" onclick="pc19WarehouseTab='progress';drawWarehouseOrders()">قيد الصرف</button>
        <button class="${pc19WarehouseTab==="done"?"active":""}" onclick="pc19WarehouseTab='done';drawWarehouseOrders()">تم الصرف</button>
        <button onclick="renderInternalIssue()">صرف داخلي</button>
